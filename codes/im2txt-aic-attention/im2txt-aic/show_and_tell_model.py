...
class ShowAndTellModel(object):
  ...
  def build_image_embeddings(self):
    ...
    print('image_embeddings0',image_embeddings)
  ...
  def attention_layer(self, features, features_proj, h, reuse=False):
      ...
      with tf.variable_scope('attention_layer', reuse=tf.AUTO_REUSE):
          ...
          out_att = tf.reshape(tf.matmul(tf.reshape(h_att, [-1, D]), w_att), [-1, L])   # (N, L)
  ...
  def build_model(self):
    ...
    with tf.variable_scope("lstm", initializer=self.initializer,reuse=tf.AUTO_REUSE):
      ...
      initial_state = (c, h)
      features_proj = self.project_features(features=self.image_embeddings)  # calculation for image features
      ...
      if self.mode == "inference":
        ...
        sequence_length = self.input_mask.get_shape()[0].value
        ...
        for time_step in range(sequence_length):
            ...
            # print(tf.concat([self.seq_embeddings[:, time_step, :], context], 1))
